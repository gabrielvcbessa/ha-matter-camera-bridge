...
if [[ -z "${MATTER_SERVER_IMAGE:-}" ]]; then
  case "$(uname -m)" in
    arm64|aarch64) export MATTER_SERVER_IMAGE=homeassistant/aarch64-addon-matter-server:9.0.3 ;;
    x86_64|amd64) export MATTER_SERVER_IMAGE=homeassistant/amd64-addon-matter-server:9.0.3 ;;
...
up() {
  ...
  wait_for_url http://127.0.0.1:18090/health sidecar
...
for name, url in checks.items():
    for attempt in range(60):
        ...
        if name != "probe" or payloads[name].get("ok"):
            break
        time.sleep(1)
