...
- Matter 1.5 live view creates the controller's `WebRtcTransportRequestor` session after the camera returns `ProvideOfferResponse`. Return the response first, then invoke `answer` and `iceCandidates` after a short delay. The lab default is 80 ms.
...
- If direct WHEP works but Matter live hangs, confirm the WHEP relay advertises a controller-reachable IP. `whep_advertise_ip` should usually equal the Home Assistant host IP. A Docker-only or loopback ICE address can make the controller allocate streams forever and then abort.
- On macOS Docker Desktop, `network_mode: host` still runs inside Docker's VM. A containerized WHEP relay can return SDP answers and tracks but never deliver RTP to a host browser. For local browser validation, run `media/whep_relay.py` on the host network, then point the sidecar at that host relay.
...
- `whep_advertise_ip` should be the same Home Assistant host IP unless there is a deliberate routed media interface. `/health` on the WHEP relay should show that value as `advertiseIp`.
- The `patricktd/MatterCameras` reference is useful for architecture: RTSP/ONVIF in, go2rtc/WebRTC relay, matter.js camera endpoints, plus optional sibling occupancy endpoints. Its generic frame-diff provider is motion-like, not true person detection; real person detection needs an external detector/provider feeding the bridge.
...
```json
{
  "advertiseIp": "192.168.68.110",
  "connectionState": "connected",
  "iceConnectionState": "completed"
}
```

In the local Home Assistant Matter Server helper, `provide_offer` can validate
allocation and WHEP forwarding while still returning Matter status `126`
(`UnsupportedAccess`) for `WebRtcTransportRequestor.answer` and
`iceCandidates`. Treat that as a helper limitation unless the real controller
logs show the same callback status.
