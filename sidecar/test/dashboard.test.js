...
test("dashboard renders PTZ controls without forbidden Home Assistant copy", () => {
  ...
  assert.match(html, /header \{ align-items: flex-start; flex-direction: column; padding: 14px 16px; gap: 12px; \}/);
  assert.match(html, /header \.primary \{ width: auto; min-width: 104px; \}/);
  assert.match(html, /main \{ padding: 12px 12px calc\(24px \+ env\(safe-area-inset-bottom\)\); gap: 12px; \}/);
  ...
  assert.match(html, /\.status-strip \{ grid-template-columns: repeat\(2, minmax\(0, 1fr\)\); gap: 8px; \}/);
  assert.match(html, /\.camera-tabs \{ flex-wrap: nowrap; overflow-x: auto;/);
  assert.match(html, /\.preview-placeholder, \.preview img, \.preview video \{ min-height: 0; \}/);
  assert.match(html, /\.camera-dialog \{ width: 100vw; height: 100dvh; max-width: 100vw; max-height: 100dvh;/);
  assert.match(html, /\.dialog-header \{ display: grid; grid-template-columns: minmax\(0, 1fr\) auto; align-items: start; \}/);
  assert.match(html, /\.dialog-header \.hint \{ display: none; \}/);
  assert.match(html, /\.dialog-footer \.button-group \{ width: 100%; display: grid; grid-template-columns: repeat\(2, minmax\(0, 1fr\)\); \}/);
  assert.match(html, /#cancel-camera-edit \{ grid-column: 1; grid-row: 1; \}/);
  assert.match(html, /#test-camera \{ grid-column: 2; grid-row: 1; \}/);
  assert.match(html, /#save \{ grid-column: 1 \/ -1; grid-row: 2; \}/);
  assert.match(html, /#dialog-footer-status:empty \{ display: none; \}/);
  assert.match(html, /\.ptz-grid button \{ min-height: 48px; \}/);
